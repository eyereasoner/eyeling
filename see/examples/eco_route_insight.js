...
const TITLE = "Eco Route Conclusion";
const EXPECTED_INPUT_FACTS = 33;
const RULES = [
  {
    "kind": "rule",
    "id": 1,
    ...
    "bodyComment": [
      ":currentRoute :fuelIndex ?Current",
      ":currentRoute :etaMinutes ?CurrentEta",
      ":altLowFuel :fuelIndex ?Alt",
      ":altLowFuel :etaMinutes ?AltEta",
      ":Policy :fuelIndexThreshold ?Threshold",
      ":Policy :maxEtaDelayMinutes ?MaxDelay",
    ...
    "headComment": [
      ...
      ":Insight :showEcoBanner \"yes\"",
      ":Insight :suggestedRoute :altLowFuel",
      ":Insight :estimatedSaving ?Saving"
    ]
  },
  {
    "kind": "rule",
      ...
      },
      {
        "s": {
          ...
          "items": [
            {
              "kind": "lit",
              "value": "=== Answer ===\nconclusion status : issue\nshow eco banner : %s\naudience : %s\nallowed use : %s\nsuggested route : %s\ncurrent fuel index : %.2f\nsuggested fuel index : %.2f\nestimated saving : %.2f\nexpires at : %s\nraw data exported : %s\nsignature algorithm : %s\npayload digest : %s\nsignature key : %s\nsignature : %s\n\n=== Explanation ===\nThe current route uses fuel index = distanceKm × (payloadKg / 1000) × gradientFactor. For %s, Current urban route gives %.2f × %.2f × %.2f = %.2f. The policy threshold is %.2f, so a local eco banner is justified. The selected alternative %s gives %.2f × %.2f × %.2f = %.2f, saving %.2f while staying within the ETA delay limit. The signed envelope exposes audience, use, expiry, route suggestion, and compact fuel indices, but not raw payload, GPS trace, driver behavior, or raw telemetry."
            },
            {
              "kind": "var",
              "value": "Show"
    ...
    "bodyComment": [
      ":Insight :status \"issue\"",
      ":Insight :showEcoBanner ?Show",
      ":Insight :suggestedRoute :altLowFuel",
      ":Insight :estimatedSaving ?Saving",
      ":Scenario :audience ?Audience",
      ":Scenario :allowedUse ?Use",
      ":Scenario :expiresAt ?Expiry",
      ":currentRoute :distanceKm ?CurrentDistance",
      ":currentRoute :gradientFactor ?CurrentGradient",
      ":currentRoute :fuelIndex ?CurrentIndex",
      ":altLowFuel :id ?AltId",
      ":altLowFuel :distanceKm ?AltDistance",
      ":altLowFuel :gradientFactor ?AltGradient",
      ":altLowFuel :fuelIndex ?AltIndex",
      ":Shipment :id ?ShipmentId",
      ":Shipment :payloadTons ?PayloadTons",
      ":Envelope :rawDataExported ?Raw",
      ":Envelope :payloadDigest ?Digest",
      ":Envelope :signatureKey ?Key",
      ":Envelope :signature ?Signature",
      ":Policy :fuelIndexThreshold ?Threshold",
      ":Policy :signatureAlgorithm ?Algorithm",
