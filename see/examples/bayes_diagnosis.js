...
const TITLE = "Bayes Diagnosis";
const EXPECTED_INPUT_FACTS = 85;
const RULES = [
  {
    "kind": "fuse",
    "id": 1,
  ...
  },
  {
    ...
    "headComment": [
      "_headBlank30 :disease ?d",
      "_headBlank30 :unnormalized ?score",
      "_headBlank30 :posterior ?post",
      ":Case :result _headBlank30",
      "?d :posterior ?post"
    ]
  },
  {
    ...
    "bodyComment": [
      "?_bodyBlank31 :disease ?d",
      "?_bodyBlank31 :posterior ?p",
      ":Case :result ?_bodyBlank31",
  ...
  },
  {
      ...
      },
      {
        "s": {
          ...
          "items": [
            {
              "kind": "lit",
              "value": "=== Answer ===\nThe most likely disease is %s (posterior = %.6f).\n\nFull posterior distribution:\n  COVID-19              posterior = %.6f  (unnormalized = %.8f)\n  Influenza             posterior = %.6f  (unnormalized = %.8f)\n  Allergic rhinitis     posterior = %.6f  (unnormalized = %.8f)\n  Bacterial pneumonia   posterior = %.6f  (unnormalized = %.8f)\n\n=== Explanation ===\nEvidence: Fever=present, DryCough=present, LossOfSmell=present, Sneezing=absent, ShortBreath=present. Evidence total (normalizing constant) = %.8f. Each posterior is prior(d) times the product of symptom likelihood factors, divided by the evidence total."
            },
            {
              "kind": "var",
              "value": "winnerLabel"
    ...
    "bodyComment": [
      ":Case :winner ?winner",
      ":Case :winnerPosterior ?winnerPost",
      ":Case :evidenceTotal ?total",
      "?winner :label ?winnerLabel",
      ":COVID19 :posterior ?covidPost",
      ":COVID19 :scoreFor ?covidScore",
      ":Influenza :posterior ?fluPost",
      ":Influenza :scoreFor ?fluScore",
      ":AllergicRhinitis :posterior ?allergyPost",
      ":AllergicRhinitis :scoreFor ?allergyScore",
      ":BacterialPneumonia :posterior ?pneuPost",
      ":BacterialPneumonia :scoreFor ?pneuScore",
...
const QUERIES = [
  {
    ...
    "premiseComment": [
      ":bayesDiagnosis :recommends ?Disease"
    ],
    "conclusionComment": [
      ":bayesDiagnosis :recommends ?Disease"
    ]
