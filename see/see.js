...
// dependency-light: generated examples should be executable with Node alone.
...
// Rules-only examples can reuse an externally authored TriG evidence file when
// it uses the same canonical slug as the generated JavaScript example.
function existingExternalInputName(name) {
  const file = path.join(INPUT_DIR, `${name}.trig`);
  return fs.existsSync(file) ? name : null;
}
