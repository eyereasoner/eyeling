...
// Convert examples/input/*.{ttl,trig,srl} -> examples/*.n3 using n3.js
...
function main() {
  ...
  // test/n3.test.js -> repo root is one level up
  const root = path.resolve(__dirname, '..');
  const examplesDir = path.join(root, 'examples');
  const inputDir = path.join(examplesDir, 'input');
  const n3JsPath = path.join(root, 'n3.js');
  ...
  if (!fs.existsSync(n3JsPath)) {
    fail(`Cannot find n3.js: ${n3JsPath}`);
    process.exit(1);
  }
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    const r = cp.spawnSync(nodePath, [n3JsPath, inPath], {
      cwd: root,
