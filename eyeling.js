...
function __normalizeDerefIri(iriNoFrag) {
    ...
    if (__IS_NODE)
        return __maybeEnforceHttps(iriNoFrag);
...
function __readFileText(pathOrFileIri) {
    ...
    if (__isFileIri(pathOrFileIri))
        path = __fileIriToPath(pathOrFileIri);
    try {
...
function __derefTextSync(iriNoFrag) {
    ...
    if (__logContentCache.has(key))
        return __logContentCache.get(key);
    let text = null;
    if (__IS_NODE) {
        if (__isHttpIri(key)) {
        ...
        if (__isHttpIri(url))
            text = __fetchHttpTextSyncBrowser(url);
    }
...
function __derefSemanticsSync(iriNoFrag) {
    const norm = __normalizeDerefIri(iriNoFrag);
    const key = typeof norm === 'string' && norm ? norm : iriNoFrag;
    if (__logSemanticsCache.has(key))
        return __logSemanticsCache.get(key);
...
function evalNumericComparisonBuiltin(g, subst, op) {
    ...
    if (aInfo && bInfo && cmpNumericInfo(aInfo, bInfo, op))
        return [{ ...subst }];
    if (g.s instanceof ListTerm && g.s.elems.length === 2) {
        ...
        if (a2 && b2 && cmpNumericInfo(a2, b2, op))
            return [{ ...subst }];
    }
    return [];
}
...
function numEqualTerm(t, n, eps = 1e-9) {
    ...
    if (Number.isNaN(v) || Number.isNaN(n))
        return false;
    // Infinity handling
...
function parseNumericLiteralInfo(t) {
    if (!(t instanceof Literal))
        return null;
    const key = t.value;
    if (__parseNumericInfoCache.has(key))
        return __parseNumericInfoCache.get(key);
    const v = key;
    const [lex, dt] = literalParts(v);
    let dt2 = dt;
    let lexStr;
...
function numericDatatypeOfTerm(t) {
    ...
    if (dt !== null) {
        ...
        if (isXsdIntegerDatatype(dt))
            return XSD_INTEGER_DT;
        if (dt === XSD_DECIMAL_DT || dt === XSD_FLOAT_DT || dt === XSD_DOUBLE_DT)
            return dt;
        return null;
    }
...
function makeNumericOutputLiteral(val, dt) {
    if (dt === XSD_INTEGER_DT) {
        if (typeof val === 'bigint')
            return internLiteral(val.toString());
        if (Number.isInteger(val))
            return internLiteral(String(val));
...
function evalUnaryMathRel(g, subst, forwardFn, inverseFn /* may be null */) {
    ...
    if (a !== null) {
        ...
        if (outDt === XSD_INTEGER_DT && !Number.isInteger(outVal))
            outDt = XSD_DECIMAL_DT;
        if (g.o instanceof Var) {
            const s2 = { ...subst };
            s2[g.o.name] = makeNumericOutputLiteral(outVal, outDt);
            return [s2];
        }
        if (g.o instanceof Blank)
            return [{ ...subst }];
        if (numEqualTerm(g.o, outVal))
            return [{ ...subst }];
        return [];
    }
    ...
    if (b !== null && typeof inverseFn === 'function') {
        ...
        if (inDt === XSD_INTEGER_DT && !Number.isInteger(inVal))
            inDt = XSD_DECIMAL_DT;
        ...
        if (g.s instanceof Blank)
            return [{ ...subst }];
        if (numEqualTerm(g.s, inVal))
            return [{ ...subst }];
        return [];
    }
    // Fully unbound: treat as satisfiable (avoid infinite enumeration)
    if (sIsUnbound && oIsUnbound)
        return [{ ...subst }];
    return [];
}
// ===========================================================================
...
function materializeRdfLists(triples, forwardRules, backwardRules) {
    ...
    for (const tr of triples) {
        ...
        if (!k)
            continue;
        if (tr.p.value === RDF_FIRST)
            firstMap.set(k, tr.o);
        else if (tr.p.value === RDF_REST)
            restMap.set(k, tr.o);
    }
    if (!firstMap.size && !restMap.size)
        return;
    ...
    function buildListForKey(k) {
        if (cache.has(k))
            return cache.get(k);
        if (visiting.has(k))
            return null; // cycle => not a well-formed list
        visiting.add(k);
        // rdf:nil => ()
        if (k === 'I:' + RDF_NIL) {
    ...
    function rewriteTerm(t) {
        ...
        if (k) {
            ...
            if (t instanceof Iri && t.value === RDF_NIL)
                return new ListTerm([]);
            return t;
        }
        if (t instanceof ListTerm) {
...
function evalBuiltin(goal, subst, facts, backRules, depth, varGen, maxResults) {
    ...
    const cryptoAlgo = pv === CRYPTO_NS + 'sha'
        ? 'sha1'
        : pv === CRYPTO_NS + 'md5'
            ? 'md5'
            : pv === CRYPTO_NS + 'sha256'
                ...
                : pv === CRYPTO_NS + 'sha512'
                    ...
                    : null;
    if (cryptoAlgo)
        return evalCryptoHashBuiltin(g, subst, cryptoAlgo);
    // -----------------------------------------------------------------
    // 4.2 math: builtins
    // -----------------------------------------------------------------
    // math:greaterThan / lessThan / notLessThan / notGreaterThan / equalTo / notEqualTo
    const mathCmpOp = pv === MATH_NS + 'greaterThan'
        ? '>'
        : pv === MATH_NS + 'lessThan'
            ? '<'
                ...
                : pv === MATH_NS + 'notGreaterThan'
                    ...
                    : pv === MATH_NS + 'equalTo'
                        ...
                        : pv === MATH_NS + 'notEqualTo'
                            ...
                            : null;
    if (mathCmpOp)
        return evalNumericComparisonBuiltin(g, subst, mathCmpOp);
    // math:sum
    ...
    if (pv === MATH_NS + 'sum') {
        ...
        if (dtOut0 === XSD_INTEGER_DT) {
            ...
            if (numEqualTerm(g.o, Number(total)))
                return [{ ...subst }];
            return [];
        }
        ...
        let dtOut = dtOut0;
        if (dtOut === XSD_INTEGER_DT && !Number.isInteger(total))
            dtOut = XSD_DECIMAL_DT;
    ...
    if (pv === MATH_NS + 'product') {
        ...
        if (dtOut0 === XSD_INTEGER_DT) {
            ...
            if (oi !== null && oi === prod)
                return [{ ...subst }];
            if (numEqualTerm(g.o, Number(prod)))
                return [{ ...subst }];
            return [];
        }
        // Numeric mode (decimal/float/double)
        ...
        let dtOut = dtOut0;
        if (dtOut === XSD_INTEGER_DT && !Number.isInteger(prod))
            dtOut = XSD_DECIMAL_DT;
    ...
    if (pv === MATH_NS + 'difference') {
        ...
        if (typeof commonNumericDatatype === 'function' && typeof makeNumericOutputLiteral === 'function') {
            ...
            if (dtOut === XSD_INTEGER_DT && !Number.isInteger(c))
                dtOut = XSD_DECIMAL_DT;
    ...
    if (pv === MATH_NS + 'exponentiation') {
        if (g.s instanceof ListTerm && g.s.elems.length === 2) {
            ...
            if (a !== null && b !== null) {
                ...
                if (dtOut === XSD_INTEGER_DT && !Number.isInteger(cVal))
                    dtOut = XSD_DECIMAL_DT;
            ...
            if (a !== null && expTerm instanceof Var && c !== null) {
                if (a > 0.0 && a !== 1.0 && c > 0.0) {
                    ...
                    if (dtB === XSD_INTEGER_DT && !Number.isInteger(bVal))
                        dtB = XSD_DECIMAL_DT;
                    const s2 = { ...subst };
    ...
    if (pv === MATH_NS + 'absoluteValue') {
        ...
        if (dtOut === XSD_INTEGER_DT && !Number.isInteger(outVal))
            dtOut = XSD_DECIMAL_DT;
        if (g.o instanceof Var) {
            const s2 = { ...subst };
    ...
    if (pv === MATH_NS + 'sinh') {
        if (typeof Math.sinh !== 'function' || typeof Math.asinh !== 'function')
            return [];
        return evalUnaryMathRel(g, subst, Math.sinh, Math.asinh);
    }
    if (pv === MATH_NS + 'cosh') {
        if (typeof Math.cosh !== 'function' || typeof Math.acosh !== 'function')
            return [];
        return evalUnaryMathRel(g, subst, Math.cosh, Math.acosh);
    }
    if (pv === MATH_NS + 'tanh') {
        if (typeof Math.tanh !== 'function' || typeof Math.atanh !== 'function')
            return [];
        ...
        for (let i = 0; i < xs.length; i++) {
            ...
            if (g.o instanceof ListTerm && g.o.elems.length === 2) {
                ...
                if (isGroundTerm(valPat2)) {
                    if (termsEqualNoIntDecimal(valPat2, val))
                        outs.push({ ...s1 });
                    continue;
                }
    ...
    if (pv === LIST_NS + 'memberAt') {
        ...
        for (let i = 0; i < xs.length; i++) {
            ...
            if (isGroundTerm(idxPat2)) {
                if (!termsEqualNoIntDecimal(idxPat2, idxLit))
                    continue;
                s1 = { ...subst };
            }
            else {
            ...
            if (isGroundTerm(o2)) {
                if (termsEqualNoIntDecimal(o2, xs[i]))
                    outs.push({ ...s1 });
                continue;
            }
    ...
    if (pv === LIST_NS + 'notMember') {
        ...
        for (const el of g.s.elems) {
            if (unifyTerm(g.o, el, subst) !== null)
                return [];
        }
        return [{ ...subst }];
    }
    ...
    if (pv === LIST_NS + 'sort') {
        ...
        if (g.s instanceof ListTerm)
            inputList = g.s.elems;
        else if (g.o instanceof ListTerm)
            inputList = g.o.elems;
        else
            return [];
        if (!inputList.every((e) => isGroundTerm(e)))
            return [];
    ...
    if (pv === LIST_NS + 'map') {
        ...
        if (!(inputTerm instanceof ListTerm))
            return [];
        const inputList = inputTerm.elems;
        if (!(predTerm instanceof Iri))
            return [];
    ...
    if (pv === LOG_NS + 'conjunction') {
        ...
        for (const part of parts) {
            ...
            if (part instanceof Literal && part.value === 'true')
                continue;
            if (!(part instanceof GraphTerm))
                return [];
    ...
    if (pv === LOG_NS + 'conclusion') {
        ...
        if (g.s instanceof GraphTerm)
            ...
            inFormula = new GraphTerm([]);
        else
            return [];
        const conclusion = __computeConclusionFromFormula(inFormula);
        if (!(conclusion instanceof GraphTerm))
            return [];
        if (g.o instanceof Var) {
            const s2 = { ...subst };
    ...
    if (pv === LOG_NS + 'rawType') {
        ...
        let ty;
        if (g.s instanceof GraphTerm)
            ty = internIri(LOG_NS + 'Formula');
        else if (g.s instanceof Literal)
            ty = internIri(LOG_NS + 'Literal');
        else if (g.s instanceof ListTerm || g.s instanceof OpenListTerm)
            ty = internIri(RDF_NS + 'List');
        else
            ty = internIri(LOG_NS + 'Other');
        if (g.o instanceof Var) {
            const s2 = { ...subst };
            s2[g.o.name] = ty;
            return [s2];
    ...
    if (pv === LOG_NS + 'dtlit') {
        ...
        if (g.o instanceof Literal) {
            ...
            if (oDt === null) {
                if (literalHasLangTag(g.o.value))
                    oDt = RDF_NS + 'langString';
                else if (isPlainStringLiteralValue(g.o.value))
                    oDt = XSD_NS + 'string';
            }
            if (oDt !== null) {
    ...
    if (pv === LOG_NS + 'langlit') {
        ...
        function extractLangTag(litVal) {
            if (typeof litVal !== 'string')
                return null;
            if (!literalHasLangTag(litVal))
                return null;
            const lastQuote = litVal.lastIndexOf('"');
            if (lastQuote < 0)
                return null;
            const after = lastQuote + 1;
            if (after >= litVal.length || litVal[after] !== '@')
                return null;
    ...
    if (pv === LOG_NS + 'includes') {
        ...
        if (g.s instanceof GraphTerm) {
            ...
            if (!snap)
                return []; // DELAY until snapshot exists
            if (lvl < prio)
                return []; // DELAY until saturated closure prio exists
            scopeFacts = snap;
        }
        // Empty formula is always included (but may be priority-gated above).
        if (g.o instanceof Literal && g.o.value === 'true')
            return [{ ...subst }];
        if (!(g.o instanceof GraphTerm))
            return [];
        const visited2 = [];
        // Start from the incoming substitution so bindings flow outward.
        return proveGoals(Array.from(g.o.triples), { ...subst }, scopeFacts, scopeBackRules, depth + 1, visited2, varGen, maxResults);
    }
    // log:notIncludes
    ...
    if (pv === LOG_NS + 'notIncludes') {
        ...
        if (!(g.o instanceof GraphTerm))
            return [];
        const visited2 = [];
        const sols = proveGoals(Array.from(g.o.triples), { ...subst }, scopeFacts, scopeBackRules, depth + 1, visited2, varGen, 1);
    ...
    if (pv === LOG_NS + 'collectAllIn') {
        ...
        if (!(clauseTerm instanceof GraphTerm))
            return [];
    ...
    if (pv === LOG_NS + 'forAllIn') {
        ...
        if (!(whereClause instanceof GraphTerm) || !(thenClause instanceof GraphTerm))
            return [];
...
function maybeCompactSubst(subst, goals, answerVars, depth) {
    ...
    if (depth < 128 && !substSizeOver(subst, 256))
        return subst;
...
function forwardChain(facts, forwardRules, backRules, onDerived /* optional */) {
            ...
            for (let i = 0; i < forwardRules.length; i++) {
                ...
                function isStrictGroundTerm(t) {
                    ...
                    if (t instanceof OpenListTerm)
                        return false;
                    if (t instanceof ListTerm)
                        return t.elems.every(isStrictGroundTerm);
                    if (t instanceof GraphTerm)
                        return t.triples.every(isStrictGroundTriple);
                ...
                for (const s of sols) {
                    ...
                    for (const cpat of r.conclusion) {
                        ...
                        const isFwRuleTriple = isLogImplies(instantiated.p) &&
                        ...
                        const isBwRuleTriple = isLogImpliedBy(instantiated.p) &&
                            ((instantiated.s instanceof GraphTerm && instantiated.o instanceof GraphTerm) ||
                                (instantiated.s instanceof GraphTerm &&
                                    instantiated.o instanceof Literal &&
                                    instantiated.o.value === 'true') ||
                        ...
                        if (isFwRuleTriple || isBwRuleTriple) {
                            ...
                            const left = instantiated.s instanceof GraphTerm
                                ? instantiated.s.triples
                                : instantiated.s instanceof Literal && instantiated.s.value === 'true'
                                    ? []
                                    : null;
                            const right = instantiated.o instanceof GraphTerm
                            ...
                            if (left !== null && right !== null) {
                                if (isFwRuleTriple) {
                                    ...
                                    const already = forwardRules.some((rr) => rr.isForward === newRule.isForward &&
                                    ...
                                    const already = backRules.some((rr) => rr.isForward === newRule.isForward &&
                                        rr.isFuse === newRule.isFuse &&
                                        triplesListEqual(rr.premise, newRule.premise) &&
                                        triplesListEqual(rr.conclusion, newRule.conclusion));
                                    if (!already) {
                        ...
                        if (!isGroundTriple(inst))
                            continue;
                        if (hasFactIndexed(facts, inst))
                            continue;
                        factList.push(inst);
    ...
    while (true) {
        ...
        if (!changedA && !changedB && scopedClosureLevel >= maxScopedClosurePriorityNeeded)
            break;
    }
    setScopedSnapshot(null, 0);
...
function termToN3(t, pref) {
    ...
    if (t instanceof Literal) {
        ...
        if (!dt)
            return t.value; // keep numbers, booleans, lang-tagged strings, etc.
        const qdt = pref.shrinkIri(dt);
        if (qdt !== null)
            return `${lex}^^${qdt}`; // e.g. ^^rdf:JSON
...
function __collectOutputStringsFromFacts(facts, prefixes) {
    ...
    for (const tr of facts) {
        if (!(tr && tr.p instanceof Iri))
            continue;
        if (tr.p.value !== LOG_NS + 'outputString')
            continue;
        if (!(tr.o instanceof Literal))
            continue;
...
function reasonStream(n3Text, opts = {}) {
    const { baseIri = null, proof = false, onDerived = null, includeInputFactsInClosure = true, enforceHttps = false, } = opts;
...
const EYELING_API = { reasonStream, version };
EYELING_API.VERSION = version;
try {
    if (typeof module !== 'undefined' && module.exports)
        module.exports = EYELING_API;
}
catch (_) { }
try {
    if (typeof self !== 'undefined')
        self.eyeling = EYELING_API;
}
catch (_) { }
function main() {
    ...
    function prefixesUsedInInputTokens(toks2, prefEnv) {
        ...
        for (let i = 0; i < toks2.length; i++) {
            ...
            if (t.typ === 'AtPrefix') {
                ...
                while (i < toks2.length && toks2[i].typ !== 'Dot' && toks2[i].typ !== 'EOF')
                    i++;
                continue;
            }
            // Skip @base ... .
    ...
    if (derived.length && usedPrefixes.length)
        console.log();
