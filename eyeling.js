...
function proveGoals(goals, subst, facts, backRules, depth, visited, varGen, maxResults, opts) {
  ...
  while (stack.length) {
    ...
    if (__treatBuiltin) {
      ...
      // If the builtin currently yields no solutions, OR it only yields *vacuous*
      // solutions (deltas without new bindings), but it still contains unbound
      // variables, try other goals first (defer).
