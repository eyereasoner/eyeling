...
function __existentializeBlankTerm(t, mapping, varGen, localBlankLabels) {
  ...
  if (t instanceof ListTerm) {
    let changed = false;
    const elems = t.elems.map((elem) => {
      const elem2 = __existentializeBlankTerm(elem, mapping, varGen, localBlankLabels);
      if (elem2 !== elem) changed = true;
      return elem2;
    });
  ...
  if (t instanceof OpenListTerm) {
    let changed = false;
    const prefix = t.prefix.map((elem) => {
      const elem2 = __existentializeBlankTerm(elem, mapping, varGen, localBlankLabels);
      if (elem2 !== elem) changed = true;
      return elem2;
    });
...
function numericTermsEqualWithinRoundoff(a, b) {
  ...
  if (ai.kind === 'bigint' || bi.kind === 'bigint') {
    return ai.kind === 'bigint' && bi.kind === 'bigint' && ai.value === bi.value;
  }
  ...
  const tolerance = 2 * Number.EPSILON * Math.max(1, Math.abs(av), Math.abs(bv));
...
function evalBuiltin(goal, subst, facts, backRules, depth, varGen, maxResults) {
  ...
  if (pv === LOG_NS + 'equalTo') {
    ...
    return numericTermsEqualWithinRoundoff(g.s, g.o) ? [__cloneSubst(subst)] : [];
  }

  // log:notEqualTo
  if (pv === LOG_NS + 'notEqualTo') {
    const s2 = unifyTerm(goal.s, goal.o, subst);
    if (s2 !== null || numericTermsEqualWithinRoundoff(g.s, g.o)) return [];
    return [__cloneSubst(subst)];
  }
...
module.exports = {
  ...
  __modules["lib/check-proof.js"] = function(require, module, exports){
...
async function main() {
  ...
  function printHelp(toStderr = false) {
    const msg =
      ...
      `      --check-proof FILE       Check a saved proof against this program.\n` +
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--check-proof' || a.startsWith('--check-proof=')) {
      let value;
      if (a === '--check-proof') {
        value = argv[i + 1];
        i += 1;
      } else {
        value = a.slice('--check-proof='.length);
      }
      if (!value || value.startsWith('-')) {
        console.error('Error: --check-proof expects a proof document.');
        process.exit(1);
      }
      argv.__checkProof = value;
      continue;
    }
  ...
  // --check-proof: re-perform every inference a saved proof records against
  // the program it claims to come from. This reasons about nothing; it only
  // verifies what the document says.
  if (argv.__checkProof) {
    const { checkProofDocument, verdict } = require('./check-proof');
    const proofText = fs.readFileSync(argv.__checkProof, 'utf8');
    const sources = sourceLabels.map((label) => ({
      text: fs.readFileSync(label === '<stdin>' ? 0 : label, 'utf8'),
      label,
    }));
    const parsed = sources.map(({ text, label }) => parseN3Text(text, { label, sourceLocations: true }));
    const program = parsed.length === 1 ? parsed[0] : mergeParsedDocuments(parsed);
    const report = checkProofDocument(program, proofText, { label: argv.__checkProof });
    for (const failure of report.failures.slice(0, 10)) {
      console.error(`  [${failure.condition}] ${failure.conclusion} -- ${failure.detail}`);
    }
    console.log(verdict(report));
    for (const [kind, count] of Object.entries(report.counts).sort()) console.log(`  ${count} ${kind}`);
...
function __setForwardRuleScopedStratumInfo(rule, level, exactLevel = false) {
  const value =
    level > 0
      ? { needsSnap: true, requiredLevel: level, exactLevel: !!exactLevel }
...
function __computeForwardRuleScopedStrata(forwardRules) {
  ...
  for (let i = 0; i < forwardRules.length; i++) {
    // Snapshot-dependent rules belong to one exact stratum: re-running a
    // negation or aggregate against a later snapshot can change its answer.
    // Ordinary monotone rules delayed by those dependencies instead have a
    // minimum stratum. Keeping them eligible at later levels ensures every new
    // scoped derivation receives its full monotone closure, even when a
    // conservative wildcard dependency orders a producer after its consumer.
    __setForwardRuleScopedStratumInfo(forwardRules[i], levels[i], metas[i].needsSnap);
...
function listLookupSpec(term) {
  if (!(term instanceof ListTerm) || term.elems.length === 0) return null;
  const positions = [];
  const keys = [];
  for (let i = 0; i < term.elems.length; i++) {
    const key = termLookupKey(term.elems[i]);
    if (key === null) continue;
    positions.push(i);
    keys.push(encodeLookupKeyPart(key));
  }
  if (positions.length === 0) return null;
  return {
    length: term.elems.length,
    positions,
    shape: term.elems.length + ':' + positions.join(','),
    key: keys.join('\u0001'),
  };
}

function demandListSubjectCandidates(facts, predicateKey, subject) {
  const spec = listLookupSpec(subject);
  if (!spec) return null;

  let byShape = facts.__deepListS.get(predicateKey);
  if (!byShape) {
    byShape = new Map();
    facts.__deepListS.set(predicateKey, byShape);
  }
  let index = byShape.get(spec.shape);
  if (!index) {
    index = new Map();
    const candidates = facts.__byPred.get(predicateKey) || EMPTY_FACT_INDEX_BUCKET;
    for (let i = 0; i < candidates.length; i++) {
      const factIndex = candidates[i];
      const factSubject = facts[factIndex].s;
      if (!(factSubject instanceof ListTerm) || factSubject.elems.length !== spec.length) continue;
      // Non-ground lists already live in __byPNonFastS and must remain in the
      // fallback bucket so that variables and protected terms stay complete.
      if (termLookupKey(factSubject) === null) continue;
      const parts = new Array(spec.positions.length);
      let usable = true;
      for (let j = 0; j < spec.positions.length; j++) {
        const key = termLookupKey(factSubject.elems[spec.positions[j]]);
        if (key === null) {
          usable = false;
          break;
        }
        parts[j] = encodeLookupKeyPart(key);
      }
      if (usable) pushMapArray(index, parts.join('\u0001'), factIndex);
    }
    byShape.set(spec.shape, index);
  }
  return index.get(spec.key) || EMPTY_FACT_INDEX_BUCKET;
}
...
function candidateFacts(facts, goal) {
  ...
  if (goal.p instanceof Iri) {
    ...
    const deepListS = sk === null ? demandListSubjectCandidates(facts, pk, goal.s) : null;
    let exact;
    if (deepListS) {
      const sBucket = mergeIndexBuckets(deepListS, byPNonFastS);
      if (ok !== null) {
        const oBucket = mergeIndexBuckets(byPO, byPNonFastO);
        exact = sBucket.length <= oBucket.length ? sBucket : oBucket;
      } else {
        exact = sBucket;
      }
    } else {
      exact = selectPositionIndexedCandidates(
...
function forwardChain(facts, forwardRules, backRules, onDerived /* optional */, opts = {}) {
  ...
  try {
    ...
    function __emitForwardRuleSolution(r, ruleIndex, s) {
      ...
      // Head blanks must be keyed on the *head's own* instantiated content, not on
      // the full instantiated body: two firings can bind different values for
      // premise-only variables (e.g. which subject matched an existential body
      // pattern) while still producing an identical instantiated head. In that
      // case a single existential witness suffices, so the head blank must come
      // out identical too (matching eye/romeo output) instead of minting a fresh
      // one per firing.
      const instantiatedHeadTriples =
        headBlankLabelsHere && headBlankLabelsHere.size ? headPatterns.map((cpat) => applySubstTriple(cpat, s)) : null;
      let headInstanceKey = null;
      function getHeadInstanceKey() {
        if (headInstanceKey === null) headInstanceKey = __firingKey(ruleIndex, instantiatedHeadTriples);
        return headInstanceKey;
      }

      for (let __hi = 0; __hi < headPatterns.length; __hi++) {
        const cpat = headPatterns[__hi];
        const instantiated = instantiatedHeadTriples ? instantiatedHeadTriples[__hi] : applySubstTriple(cpat, s);
        ...
        if (isFwRuleTriple || isBwRuleTriple) {
          ...
          if (isFwRuleTriple) {
            // A rule the engine generates here is in no document, so it gets
            // no source: carrying the producing rule's would make a proof
            // cite a position that holds a different rule. A proof carries
            // such a rule itself instead (see `ruleReference`).
  ...
  return __withScopedSnapshotForQueries(facts, () => {
    for (let qi = 0; qi < logQueryRules.length; qi++) {
      ...
      for (const s of sols) {
        ...
        // Key head-blank skolemization on the head's own instantiated content
        // (see forwardChain's __emitForwardRuleSolution for why: two firings can
        // differ only in premise-only bindings yet produce an identical head).
        const instantiatedHeadTriples =
          headBlankLabelsHere && headBlankLabelsHere.size
            ? headPatterns.map((cpat) => applySubstTriple(cpat, s))
            : null;
        const fireKey = instantiatedHeadTriples ? __firingKey(1000000 + qi, instantiatedHeadTriples) : null;
...
const { parseN3Text } = require('./multisource');
const { checkProofDocument, readProofDocument, verdict: proofVerdict } = require('./check-proof');
...
// A variable the engine minted for a blank node carries a private-use
// character in its name so it cannot collide with one a user wrote. That
// character has no place in output: a document carrying it is not N3 any
// more, and cannot be read back.
function printableVarName(name) {
  return isInternalBlankVarName(name) ? `eyeling_b${internalBlankVarSuffix(name)}` : name;
}
...
function termToN3(t, pref) {
  ...
  if (t instanceof Var) return `?${printableVarName(t.name)}`;
